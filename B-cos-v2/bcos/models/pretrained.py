...
def _iter_weight_layout_candidates(root: Path, filename: str) -> Iterable[Path]:
    yield root / filename
    yield root / "weights" / filename
    yield root / "weights" / "bcos-imagenet" / filename
    yield root / "weights" / "bcos-imagenet.zip"
    yield root / "bcos-weights" / filename
    yield root / "bcos-imagenet" / filename
    yield root / "bcos-imagenet.zip"


def _iter_local_weight_candidates(filename: str) -> Iterable[Path]:
    env_file = os.environ.get("BCOS_WEIGHT_FILE")
    if env_file:
        yield Path(env_file)
    ...
    for env_name in ("BCOS_WEIGHTS_DIR", "MODEL_WEIGHTS_DIR", "WEIGHTS_DIR"):
        ...
        if env_dir:
            yield from _iter_weight_layout_candidates(Path(env_dir), filename)

    module_path = Path(__file__).resolve()
    repo_root = module_path.parents[2]
    yield from _iter_weight_layout_candidates(repo_root, filename)

    kaggle_working = Path("/kaggle/working")
    if kaggle_working.is_dir():
        yield from _iter_weight_layout_candidates(kaggle_working, filename)

    kaggle_input = Path("/kaggle/input")
    if kaggle_input.is_dir():
        yield from _iter_weight_layout_candidates(kaggle_input, filename)
        for dataset_dir in sorted(p for p in kaggle_input.rglob("*") if p.is_dir()):
            if dataset_dir.name in {
                ...
                yield from _iter_weight_layout_candidates(dataset_dir, filename)
...
def _torch_load_bytes(data: bytes) -> Dict[str, Any]:
    try:
...
def _load_from_zip(zip_path: Path, filename: str) -> Dict[str, Any] | None:
    with zipfile.ZipFile(zip_path) as archive:
        preferred = [
            filename,
            f"bcos-imagenet/{filename}",
            f"weights/bcos-imagenet/{filename}",
        ]
...
def _load_pretrained_state_dict(url: str, progress: bool) -> Dict[str, Any]:
    filename = Path(urlparse(url).path).name
    for candidate in _iter_local_weight_candidates(filename):
        if candidate.is_file():
            if candidate.suffix.lower() == ".zip":
                state_dict = _load_from_zip(candidate, filename)
                if state_dict is None:
                    continue
                print(f"Loading local B-cos weights from {candidate}:{filename}")
                return state_dict

            print(f"Loading local B-cos weights from {candidate}")
    ...
    try:
        ...
        searched = "\n".join(f"  - {path}" for path in _iter_local_weight_candidates(filename))
        raise RuntimeError(
            "Could not load B-cos pretrained weights offline.\n"
            f"Expected local file named '{filename}' in one of:\n{searched}\n"
            "Attach a Kaggle dataset containing the weight files and/or set BCOS_WEIGHTS_DIR."
        ) from exc
...
def _get_model(
    ...
    **model_kwargs: Any,
    ...
    if pretrained:
        ...
        state_dict = _load_pretrained_state_dict(url, progress)
