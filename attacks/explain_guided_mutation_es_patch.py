...
WEAKEST_DROP_MUTATION_PROBABILITY = 0.1
WEAKEST_DROP_MUTATION_FRACTION = 0.1


def apply_weakest_drop_mutation(
    patches: Tensor,
    mutation_probability: float = WEAKEST_DROP_MUTATION_PROBABILITY,
    drop_fraction: float = WEAKEST_DROP_MUTATION_FRACTION,
) -> Tensor:
    if (
        mutation_probability <= 0.0
        or drop_fraction <= 0.0
        or patches.numel() == 0
    ):
        return patches

    mutated = patches.clone()
    flat_patches = mutated.reshape(-1, *mutated.shape[-3:])
    if flat_patches.shape[0] == 0:
        return mutated

    spatial_strength = flat_patches.square().sum(dim=1).sqrt().reshape(
        flat_patches.shape[0],
        -1,
    )
    mutate_mask = torch.rand(flat_patches.shape[0], device=flat_patches.device) < mutation_probability
    spatial_size = spatial_strength.shape[1]

    for patch_idx in torch.nonzero(mutate_mask, as_tuple=False).flatten():
        strength = spatial_strength[patch_idx]
        active_idx = torch.nonzero(strength > 0, as_tuple=False).flatten()
        if active_idx.numel() == 0:
            continue

        drop_count = min(int(active_idx.numel() * drop_fraction), spatial_size)
        if drop_count <= 0:
            continue

        weakest_rel_idx = torch.topk(
            strength[active_idx],
            k=drop_count,
            largest=False,
            sorted=False,
        ).indices
        weakest_idx = active_idx[weakest_rel_idx]
        keep_mask = torch.ones(
            spatial_size,
            dtype=flat_patches.dtype,
            device=flat_patches.device,
        )
        keep_mask[weakest_idx] = 0.0
        flat_patches[patch_idx] = flat_patches[patch_idx] * keep_mask.view(
            1,
            flat_patches.shape[-2],
...
class ExplainGuidedPatchES:
    ...
    def ask(self) -> Tensor:
        ...
        return apply_weakest_drop_mutation(candidates)
...
class BatchedExplainGuidedPatchES:
    ...
    def ask(self) -> Tensor:
        ...
        return apply_weakest_drop_mutation(flat_candidates.view_as(candidates))
