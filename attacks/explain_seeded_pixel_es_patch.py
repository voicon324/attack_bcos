...
This variant uses `model.explain()` to choose the patch location from
the top-1 predicted class and to seed the first ES generation with
patches cropped from the explanations of the top-5 predicted classes.
All explain-derived seed patches share the top-1 patch position, then
the attack continues with the same fixed-position ES optimization under
an l0/l2/linf budget.
"""
...
def resolve_patch_positions(
    ...
    if use_explain_position:
        primary_pos = find_best_patch_position(
...
def build_seed_patch_from_explanation(
    explanation: Any,
    patch_size: int,
    pos_y: int,
    pos_x: int,
    ...
    explanation_arr = np.asarray(explanation)
    if explanation_arr.ndim != 3 or explanation_arr.shape[2] < 4:
        raise ValueError(
            "Expected explanation image shaped like (H, W, 4) or compatible RGBA array."
        )

    height, width = explanation_arr.shape[:2]
    if patch_size > height or patch_size > width:
        raise ValueError(
            f"Patch size {patch_size} exceeds explanation size {(height, width)}."
        )

    crop_y = min(max(int(pos_y), 0), height - patch_size)
    crop_x = min(max(int(pos_x), 0), width - patch_size)
    crop = torch.as_tensor(
        explanation_arr[crop_y:crop_y + patch_size, crop_x:crop_x + patch_size, :4],
        device=device,
        dtype=dtype,
    )
    rgb = crop[..., :3]
    alpha = crop[..., 3:4]
    return (rgb * alpha).permute(2, 0, 1).unsqueeze(0).contiguous()


def prepare_explain_seed_patches(
    ...
    class_list = [
        int(class_idx)
        for class_idx in topk_classes.detach().to(dtype=torch.long, device="cpu").view(-1).tolist()
    ]
    if not class_list:
        raise ValueError("Expected at least one class to prepare explain seed patches.")
    ...
    primary_pos = patch_positions[0]

    seed_patches: List[Tensor] = []
    for class_offset, class_idx in enumerate(class_list):
        guidance = primary_guidance
        if class_offset != 0:
            guidance = extract_attribution(
                guide_model,
                x_bcos,
                target_class=class_idx,
            )
        seed_patch = build_seed_patch_from_explanation(
        ...
        seed_patches.append(seed_patch)

    stacked_seed_patches = torch.stack(seed_patches, dim=0)
    stacked_seed_patches = project_perturbation(stacked_seed_patches, epsilon, norm)
    return patch_positions, stacked_seed_patches
...
class ExplainGuidedPatchES:
    def __init__(
        ...
        norm: str,
        device: torch.device,
        initial_seed_patches: Optional[Tensor] = None,
    ):
        ...
        self.initial_seed_patches: Optional[Tensor] = None
        self.used_initial_seed_patches = False
        if initial_seed_patches is not None:
            if initial_seed_patches.dim() != 5 or initial_seed_patches.shape[1:] != self.param_shape:
                raise ValueError(
                    "initial_seed_patches must have shape (seed_count, 1, 3, patch_size, patch_size)."
                )
            seed_patches = initial_seed_patches.to(
                device=device,
                dtype=self.perturbation.dtype,
            )
            seed_patches = project_perturbation(seed_patches, self.epsilon, self.norm)
            self.initial_seed_patches = seed_patches.clone()
    ...
    def ask(self) -> Tensor:
        ...
        candidates = project_perturbation(candidates, self.epsilon, self.norm)
        if (
            not self.used_initial_seed_patches
            and self.initial_seed_patches is not None
            and self.initial_seed_patches.numel() > 0
        ):
            seed_count = min(self.population_size, self.initial_seed_patches.shape[0])
            candidates[:seed_count] = self.initial_seed_patches[:seed_count]
            self.used_initial_seed_patches = True
        return candidates
...
class BatchedExplainGuidedPatchES:
    def __init__(
        ...
        if initial_seed_patches is not None:
            if (
                initial_seed_patches.dim() != 6
                or initial_seed_patches.shape[0] != batch_size
                or initial_seed_patches.shape[2:] != self.patch_shape
            ):
                raise ValueError(
                    "initial_seed_patches must have shape "
                    "(batch_size, seed_count, 1, 3, patch_size, patch_size)."
                )
            seed_patches = initial_seed_patches.to(
                device=device,
                dtype=self.perturbation.dtype,
            )
            seed_patches = project_perturbation(
                seed_patches.reshape(-1, *self.patch_shape),
                self.epsilon,
                self.norm,
            ).reshape_as(seed_patches)
            self.initial_seed_patches = seed_patches.clone()
    ...
    def ask(self) -> Tensor:
        ...
        candidates = flat_candidates.view_as(candidates)
        if (
            not self.used_initial_seed_patches
            and self.initial_seed_patches is not None
            and self.initial_seed_patches.numel() > 0
        ):
            seed_count = min(self.population_size, self.initial_seed_patches.shape[1])
            candidates[:, :seed_count] = self.initial_seed_patches[:, :seed_count]
            self.used_initial_seed_patches = True
        return candidates
...
def run_explain_guided_attack(
    ...
    patch_positions, initial_seed_patches = prepare_explain_seed_patches(
    ...
    es = ExplainGuidedPatchES(
        ...
        norm=norm,
        device=x_rgb.device,
        initial_seed_patches=initial_seed_patches,
    )
...
def run_explain_guided_attack_batch(
    ...
    patch_positions: List[List[Tuple[int, int]]] = []
    initial_seed_patch_list: List[Tensor] = []
    for image_idx in range(batch_size):
        positions, seed_patches = prepare_explain_seed_patches(
            guide_model=guide_model,
            x_rgb=x_rgb[image_idx:image_idx + 1],
            topk_classes=topk_classes[image_idx],
            patch_size=patch_size,
            pos_y=pos_y,
            pos_x=pos_x,
            use_explain_position=use_explain_position,
        ...
        initial_seed_patch_list.append(seed_patches)

    initial_seed_patches = torch.stack(initial_seed_patch_list, dim=0)
...
def process_single_image(
    ...
    with torch.inference_mode():
        ...
        topk_count = min(5, out_orig.shape[1])
        topk_logits, topk_classes = out_orig[0].topk(topk_count)
        target_class = int(topk_classes[0].item())
        logit_orig = float(topk_logits[0].item())

    print(f"  Original class: {target_class} (logit: {logit_orig:.3f})")
    if topk_count > 1:
        print(f"  Runner-up class: {int(topk_classes[1].item())}")
...
def process_image_batch(
    ...
    with torch.inference_mode():
        ...
        topk_count = min(5, out_orig.shape[1])
        topk_logits, topk_classes = out_orig.topk(topk_count, dim=1)
        target_classes = topk_classes[:, 0]
        logit_orig = topk_logits[:, 0]
