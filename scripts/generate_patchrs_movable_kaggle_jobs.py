...
def iter_jobs() -> Iterable[dict]:
    for model in MODELS:
        for position in POSITIONS:
            job_id = (
                f"patchrs-bcos-movable-{model}-s{PATCH_SIZE}-"
                f"linf{linf_slug(LINF)}-init-{position}"
            )
            yield {
                "job_id": job_id,
                "title": f"Patch-RS B-cos movable {model} s{PATCH_SIZE} {LINF} init {position}",
                ...
                "job_config": {
                    ...
                    "save_images": False,
                    "alpha_init": 0.4,
                    "patch_init": "random_squares",
                    "patch_init_iters": 1000,
                    "code_dataset_owner": "hkhnhduy",
                    "code_dataset_slug": "attack-bcos-github",
                    "experiment": "patchrs_movable_s16_linf64",
                },
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate movable-position Patch-RS B-cos Kaggle jobs."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("kaggle/patchrs_movable_s16_linf64_jobs.json"),
