            ...
            for linf in LINFS:
                for position in POSITIONS:
                    if position == "gradcam" and model in VITC_MODELS:
                        continue
                    job_id = f"lavan-bcos-{model}-s{patch_size}-linf{linf_slug(linf)}-{position}"
                    yield {
                        "job_id": job_id,
                        "title": f"LaVAN B-cos {model} s{patch_size} {linf} {position}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate LaVAN B-cos Kaggle job config.")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("kaggle/lavan_jobs.json"),
