...
def iter_jobs(queries: int) -> Iterable[dict]:
    for model in MODELS:
        for position in POSITIONS:
            job_id = (
                f"adversarialpatch-bcos-movable-{model}-s{PATCH_SIZE}-"
                f"linf{linf_slug(LINF)}-init-{position}"
            )
            yield {
                "job_id": job_id,
                "title": f"Adversarial Patch B-cos movable {model} s{PATCH_SIZE} {LINF} init {position}",
                ...
                "job_config": {
                    ...
                    "experiment": "adversarialpatch_movable_s16_linf64",
                },
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate movable-position A-LinCui Adversarial Patch B-cos Kaggle jobs."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("kaggle/adversarialpatch_movable_s16_linf64_jobs.json"),
